...
export class UploadBtnComponent implements OnInit, OnChanges, OnDestroy {
  ...
  uploadFiles(fileList: FileList | File[]): void {
    ...
    filters$.subscribe(
      ...
      e => {
        console.log(`Unhandled upload filter error`, e);
      }
    );
  }
  ...
  private upload(file: UploadFile, fileList: UploadFile[]): void {
    ...
    if (before instanceof Observable) {
      before.subscribe(
        ...
        e => {
          console.log(`Unhandled upload beforeUpload error`, e);
        }
      );
  ...
  private post(file: UploadFile): void {
    ...
    if (!(req$ instanceof Subscription)) {
      console.log(`Must return Subscription type in '[nzCustomRequest]' property`);
    }
    this.reqs[uid] = req$;
    // tslint:disable-next-line:no-non-null-assertion
    opt.onStart!(file);
